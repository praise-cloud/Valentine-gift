...
export function Gallery() {
    ...
    const animationDetails = React.useMemo(() => {
        ...
        const singleImageDuration = 2; // Fast upward motion (2 seconds per image)
        const repeatDelay = (totalImages - 1) * singleImageDuration;

        return images.map((img, index) => ({
            ...img,
            x: ((index * 137) % 80) + 10,
            rotate: ((index * 151) % 20) - 10,
            delay: index * singleImageDuration, // Each image starts after the previous one
            duration: singleImageDuration, // Each animation lasts 2 seconds
            repeatDelay: repeatDelay, // Wait for all other images before repeating
        }))
    }, [])
